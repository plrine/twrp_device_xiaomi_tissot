...
if mount -o ro /dev/block/bootdevice/by-name/userdata /data; then
    ...
    if [ -d '/data/media/0/Android' ]; then
        ...
        # FBE
        sed -i 's|,encryptable=footer|,fileencryption=aes-256-xts:aes-256-cts:v2|g' /system/etc/recovery.fstab
        sed -i 's|encryptable=footer;|fileencryption=aes-256-xts:aes-256-cts:v2;|g' /system/etc/twrp.flags
    fi
    umount /data || true
fi
